...
`app/layout.tsx` กำหนด `title`, `description`, `icons` แบบคงที่ และตั้ง `<html lang="th">` ส่วน `next.config.mjs` อนุญาต remote images จาก supabase/unsplash/google แล้ว และมี `NEXT_PUBLIC_SITE_URL=https://kawan2.app` ใน env
...
export async function generateMetadata({ params }): Promise<Metadata> {
  ...
  const { data: n } = await supabase
    .from("news")
    .select(
      "title, body, cover_url, category, status, published_at, profiles(display_name)",
    )
    .eq("slug", params.slug)
    .single();

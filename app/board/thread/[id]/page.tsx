...
export default async function ThreadPage({
  ...
  const { data: thread } = await supabase
    .from("threads")
    .select(
      "id, title, body, author_id, updated_at, like_count, reply_count, view_count, created_at, is_locked, is_pinned, profiles(username, display_name, avatar_url, level_id, role, reputation, created_at, bio, threads(count), posts(count)), categories(name_th, slug)",
    )
    .eq("id", threadId)
  ...
  const { data: posts } = await supabase
    .from("posts")
    .select(
      "id, body, author_id, updated_at, like_count, created_at, profiles(username, display_name, avatar_url, level_id, role, reputation, created_at, bio, threads(count), posts(count))",
    )
  ...
  if (user) {
    ...
    const [{ data: me }, { data: tReact }, { data: pReacts }] =
      await Promise.all([
  ...
  return (
    <div className="w-full space-y-4">
      <JsonLd
        data={{
          ...
          author: tAuthor
            ? {
                "@type": "Person",
                name: tAuthor.display_name || tAuthor.username,
              }
            : undefined,
        ...
        data={{
          ...
          itemListElement: [
              ...
              : []),
            {
              ...
              position: cat ? 4 : 3,
              name: thread.title,
              item: `${SITE}/board/thread/${threadId}`,
            },
          ],
        }}
      />
      ...
      <nav className="flex flex-wrap items-center gap-1 text-xs text-on-surface-variant">
        ...
        {cat && (
          <>
            <Link href={`/board/${cat.slug}`} className="hover:text-primary">
              {cat.name_th}
            </Link>
            <span>›</span>
          </>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:justify-end">
          {isAdmin && (
            <form action={setThreadPin}>
              ...
              <input
                type="hidden"
                name="pinned"
                value={thread.is_pinned ? "0" : "1"}
              />
              <button className="btn-outline gap-1">
                {thread.is_pinned ? (
                  <>
                    <PinOff className="h-4 w-4" /> ยกเลิกปักหมุด
                  </>
                ) : (
                  <>
                    <Pin className="h-4 w-4" /> ปักหมุด
                  </>
                )}
              </button>
            </form>
          ...
          {canEditThread && (
            <Link
              href={`/board/thread/${threadId}/edit`}
          ...
          <a href="#reply" className="btn-primary gap-1">
            <CornerUpLeft className="h-4 w-4" /> ตอบกระทู้
          </a>
        </div>
      </div>
      ...
      <article className="card flex flex-col sm:flex-row overflow-hidden">
        ...
        <div className="min-w-0 flex-1 p-4 sm:p-5">
          <h1 className="flex items-start gap-2 text-xl font-bold text-on-surface sm:text-2xl">
            {thread.is_pinned && (
              <Pin className="mt-1 h-5 w-5 shrink-0 text-tertiary-container" />
            )}
            {thread.title}
          </h1>
          <p className="mt-1 text-xs text-on-surface-variant">
            โพสต์เมื่อ {new Date(thread.created_at).toLocaleString("th-TH")} ·{" "}
            {thread.view_count} อ่าน
            {threadEdited && (
              <span className="ml-1 italic">
                (แก้ไขเมื่อ{" "}
                {new Date((thread as any).updated_at).toLocaleString("th-TH")})
              </span>
            )}
          </p>
          <div
            className="bbcode mt-3 text-on-surface"
            dangerouslySetInnerHTML={{ __html: renderBBCode(thread.body) }}
          />
          <div className="mt-5 flex items-center gap-4 border-t border-outline-variant pt-3 text-sm text-on-surface-variant">
            <form action={reactThread}>
              <input type="hidden" name="thread_id" value={threadId} />
              <button
                className={`inline-flex items-center gap-1 ${likedThread ? "font-medium text-primary" : "hover:text-primary"}`}
              >
                <Heart
                  className={`h-4 w-4 ${likedThread ? "fill-current" : ""}`}
                />{" "}
                ถูกใจ ({thread.like_count})
              </button>
            </form>
            <a
              href="#reply"
            ...
            >
              <CornerUpLeft className="h-4 w-4" /> ตอบกลับ
            </a>
      ...
      <div className="space-y-3">
            ...
            >
              ...
              <div className="min-w-0 flex-1 p-4">
                ...
                {isEditing ? (
                  <form
                    action={updatePost}
                    className="mt-2 overflow-hidden rounded border border-outline-variant"
                  >
                  ...
                  <div
                    className="bbcode mt-2 text-on-surface"
                    dangerouslySetInnerHTML={{ __html: renderBBCode(p.body) }}
                  />
                )}
                {!isEditing && (
                  <div className="mt-3 flex items-center gap-4 text-xs text-on-surface-variant">
                    <form action={reactPost}>
                      ...
                      <input type="hidden" name="thread_id" value={threadId} />
                      <button
                        className={`inline-flex items-center gap-1 ${likedPosts.has(p.id) ? "font-medium text-primary" : "hover:text-primary"}`}
                      >
                        <Heart
                          className={`h-3.5 w-3.5 ${likedPosts.has(p.id) ? "fill-current" : ""}`}
                        />{" "}
                        {p.like_count} ถูกใจ
                      </button>
                    </form>
        ...
        {(!posts || posts.length === 0) && (
          <p className="card p-6 text-center text-sm text-on-surface-variant">
            ยังไม่มีความเห็น เป็นคนแรกที่ตอบกลับ
          </p>
        )}
      </div>
      ...
      <div id="reply" className="scroll-mt-20">
        {thread.is_locked ? (
          ...
          <form action={createReply} className="card overflow-hidden">
            <div className="bg-primary px-4 py-2.5 text-sm font-semibold text-on-primary">
              ตอบกระทู้
            </div>
            <input type="hidden" name="thread_id" value={threadId} />

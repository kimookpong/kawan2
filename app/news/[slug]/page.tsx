...
export async function generateMetadata({
  ...
  return {
    ...
    robots:
      n.status === "published" ? undefined : { index: false, follow: false },
  };
}
...
export default async function NewsDetailPage({
  ...
  const { data: news } = await supabase
    .from("news")
    .select(
      "id, title, body, cover_url, category, status, view_count, published_at, profiles(username, display_name, avatar_url, level_id, role, reputation, created_at, threads(count), posts(count))",
    )
    .eq("slug", params.slug)
  ...
  const { data: comments } = await supabase
    .from("news_comments")
    .select(
      "id, body, created_at, profiles(username, display_name, avatar_url, level_id, role, reputation, created_at, threads(count), posts(count))",
    )
    .eq("news_id", news.id)
    .eq("status", "published")
    .eq("profiles.threads.status", "published")
  ...
  return (
    <div className="w-full space-y-4">
      <JsonLd
        data={{
          ...
          publisher: {
            "@type": "Organization",
            name: "Kawan2",
            logo: { "@type": "ImageObject", url: `${SITE}/image.png` },
          },
      ...
      <nav className="flex flex-wrap items-center gap-1 text-xs text-on-surface-variant">
        ...
        {news.category && (
          <>
            <span className="text-on-surface-variant">{news.category}</span>
            <span>›</span>
          </>
        )}
      ...
      {news.status !== "published" && (
        <p className="rounded border border-amber-400/50 bg-amber-50 px-3 py-2 text-sm text-amber-800">
          ข่าวนี้เป็น “{news.status === "draft" ? "ฉบับร่าง" : news.status}” —
          ยังไม่แสดงต่อสาธารณะ (เห็นเฉพาะบรรณาธิการ/แอดมิน)
        </p>
      )}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:justify-end">
          ...
          <a href="#comments" className="btn-primary gap-1">
            <MessageSquare className="h-4 w-4" /> แสดงความเห็น
          </a>
        </div>
      </div>
        ...
        <div className="min-w-0 flex-1 p-4 sm:p-5">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-on-surface-variant">
            ...
            <span>
              เผยแพร่เมื่อ{" "}
              {news.published_at &&
                new Date(news.published_at).toLocaleString("th-TH")}
              {typeof news.view_count === "number"
                ? ` · ${news.view_count} อ่าน`
                : ""}
            </span>
          </div>
          ...
          {news.cover_url && (
            ...
            <img
              src={news.cover_url}
              alt=""
              className="mt-3 w-full max-w-[640px] rounded-lg object-cover"
            />
          )}

          <div
            className="bbcode mt-4 text-on-surface"
            dangerouslySetInnerHTML={{ __html: renderBBCode(news.body) }}
          />
          ...
          <div className="mt-5 flex items-center gap-4 border-t border-outline-variant pt-3 text-sm text-on-surface-variant">
            ...
            >
              <MessageSquare className="h-4 w-4" /> ความเห็น (
              {comments?.length ?? 0})
            </a>
      ...
      <h2
        id="comments"
        className="scroll-mt-20 border-b border-outline-variant pb-2 text-lg font-semibold"
      >
          ...
          >
            ...
            <div className="min-w-0 flex-1 p-4">
              ...
              <div
                ...
                dangerouslySetInnerHTML={{ __html: renderBBCode(c.body) }}
              />
              <div className="mt-3 flex items-center gap-4 text-xs text-on-surface-variant">
                <QuoteButton
                  author={
                    c.profiles?.display_name || c.profiles?.username || "สมาชิก"
                  }
                  body={c.body}
                />
                <div className="ml-auto">
      ...
      <div className="scroll-mt-20">
        {user ? (
          ...
          <p className="card p-4 text-center text-sm text-on-surface-variant">
            กรุณา
            <Link
              href={`/auth/login?redirect=/news/${params.slug}`}
